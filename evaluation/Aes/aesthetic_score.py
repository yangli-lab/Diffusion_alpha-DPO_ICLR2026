...
class Aes_Predictor:
    def __init__(self, device):
        ...
        s = torch.load("./evaluation/Aes/sac+logos+ava1-l14-linearMSE.pth")   # load the model you trained previously or the model available in this repo
    ...
    def score(self, img_path):
        if type(img_path) == list:
            ...
            for img in img_path:
                ...
                im = self.preprocess(read_img).unsqueeze(0).to(self.device)
                pil_image.append(im)
            image = torch.concat(pil_image, dim=0)
        elif type(img_path) == str:
...
if __name__ == "__main__":
    aes_predictor = Aes_Predictor("cuda")
    img_path = ["test.png"] * 3
    aes_score = aes_predictor.score(img_path)
    print(aes_score)