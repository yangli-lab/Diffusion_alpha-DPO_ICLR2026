...
def parse_args():
    ...
    parser.add_argument("--alpha_dpo", type=float, default=0.9999, help="The beta DPO temperature controlling strength of KL penalty")
    parser.add_argument(
    ...
    parser.add_argument(
        "--reweight_step", type=int, default=600, help="Only train on pairs where both generations are from dreamlike"
    )
...
def main():
    ...
    dataset = load_dataset(
            "json",
            data_files=args.dataset_name, # json file
        )
    ...
    if 'pickapic' in args.dataset_name or 'pick2pic' in args.dataset_name or (args.train_method == 'dpo'):
        pass
    ...
    if args.train_method == 'dpo':
        ...
        def preprocess_train(examples):
            ...
            for col_name in ['jpg_0', 'jpg_1']:
                images = [Image.open(os.path.join(args.train_data_dir, img_url)).convert("RGB") for img_url in examples[col_name]]
        ...
        def preprocess_train(examples):
            if 'pickapic' in args.dataset_name or 'pick2pic' in args.dataset_name:
                images = []
                ...
                for im_0_bytes, im_1_bytes, label_0 in zip(examples['jpg_0'], examples['jpg_1'], examples['label_0']):
                    ...
                    images.append(Image.open(os.path.join(args.train_data_dir, im_bytes)).convert("RGB"))
            else:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'sft': # SFT, casting for F.mse_loss
                    ...
                    implicit_acc = ((model_diff - ref_diff) < 0).sum().float() / (model_diff - ref_diff).size(0)
                    inside_term = scale_term * (model_diff - ref_diff)
                    sigma_as_reweight = F.sigmoid(inside_term)

                    if args.reweight_step >= global_step:
                        reweight_all = accelerator.gather(reweight)
                        global_mean = reweight_all.mean() 
                        p = F.sigmoid(scale_term * (model_diff - ref_diff))
                        reverse_p = F.sigmoid(-1 * scale_term * (model_diff - ref_diff))
                        sigma_as_reweight_all = accelerator.gather(sigma_as_reweight.detach())
                        sigma_as_reweight_global_mean = sigma_as_reweight_all.mean()
                        alpha = args.alpha_dpo
                        loss1 = ((1 / (alpha * (alpha - 1))) * (p.pow(alpha) - (1 - alpha) - alpha * p)).mean()
                        loss = loss1 
                    elif args.reweight_step < global_step:
                        reweight_all = accelerator.gather(reweight)
                        global_mean = reweight_all.mean()
                        p = F.sigmoid(scale_term * (model_diff - ref_diff))
                        reverse_p = F.sigmoid(-1 * scale_term * (model_diff - ref_diff))
                        sigma_as_reweight_all = accelerator.gather(sigma_as_reweight.detach())
                        sigma_as_reweight_global_mean = sigma_as_reweight_all.mean()
                        alpha = args.alpha_dpo * sigma_as_reweight_global_mean.detach()
                        loss1 = ((1 / (alpha * (alpha - 1))) * (p.pow(alpha) - (1 - alpha) - alpha * p)).mean()
                        loss = loss1
                    else:
                        raise ValueError(f'global step error: {global_step}')
            ...
            if args.train_method == 'dpo':
                logs["ref_loss"] = raw_ref_loss.detach().item()
                logs["raw_model_loss"] = raw_model_loss.detach().item()
